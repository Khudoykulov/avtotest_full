...
class APIConfig(models.Model):
    ...
    # Kunlik tracking
    used_tokens_daily = models.IntegerField(default=0)
    ...
    def reset_if_needed(self):
        ...
        if self.last_daily_reset < today:
            self.used_tokens_daily = 0
            self.last_daily_reset = today
            self.save()

    def is_available(self):
        self.reset_if_needed()
        return self.is_active and self.used_tokens_daily < 100000

    def add_used_tokens(self, tokens):
        self.used_tokens_daily += tokens
        self.save()

    def __str__(self):
        return f"API Key #{self.priority}"