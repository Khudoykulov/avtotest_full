...
def test_results_view(request, result_id):
    ...
    time_taken_minutes = int(test_result.time_taken.total_seconds() // 60)
    time_taken_seconds = int(test_result.time_taken.total_seconds() % 60)
    context = {
...
def ai_analysis_view(request):
    ...
    if request.method == 'POST':
        try:
            ...
            if not test_result_id:
                return JsonResponse({
                    'success': False,
                    'error': 'Test result ID korsatilmagan'
                })

            test_result = get_object_or_404(TestResult, id=test_result_id, user=request.user)

            # Groq AI tahlilini olish
            from .ai_analytics import get_ai_analysis_for_single_test
            ai_result = get_ai_analysis_for_single_test(test_result)
            ...
            ai_text = ai_result.get('ai_analysis', '')

            if not ai_text:
                return JsonResponse({
                    'success': False,
                    'error': 'AI javobi bosh'
                })
            ...
            ai_analysis_obj = AIAnalysis.objects.create(
                ...
                recommendations=recommendations,
                confidence_score=ai_result.get('confidence', 85),
    ...
    return JsonResponse({'success': False, 'error': 'Faqat POST qabul qilinadi'})